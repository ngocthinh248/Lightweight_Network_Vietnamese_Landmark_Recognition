...
parser.add_argument('--mode', default='C',
                    help="Defines the distill mode: B or C, default is C B: use the universial dataset of place365 with 365 labels for the dataset input.\\                    C: use the local dataset with the number of optional labels,if you choose mode C, please pass the parameter --num_classes")
parser.add_argument('--num_classes', default='30',
                    help="The number of labels for the local dataset in case of mode C")
parser.add_argument('--model_teacher', default='resnet18',
                    help="The architecture of the model teacher: resnet18 or resnet50, the default is resnet18")
parser.add_argument('--model_student', default='3CNN',
                    help="identify the student model: 3CNN or 5CNN or 7CNN, default: 3CNN")
...
parser.add_argument('--data_dir', default=None,
                    help="Directory dataset of place365")
...
if __name__ == '__main__':
    ...
    json_path = os.path.join(args.model_dir, 'params.json') # lấy param
    ...
    metrics = net.metrics  
    if args.mode == 'B':
        if ((args.model_teacher == 'resnet18') or (args.model_teacher != 'resnet50')):
            teacher_model = wideresnet.resnet18(num_classes = 365)
            teacher_checkpoint = 'experiments/base_resnet18/wideresnet18_places365.pth.tar'
            teacher_model = teacher_model.cuda() if params.cuda else teacher_model
            utils.load_checkpoint(teacher_checkpoint, teacher_model)
        elif args.model_teacher == 'resnet50':
            teacher_model = wideresnet.resnet50(num_classes = 365)
            teacher_checkpoint = 'experiments/base_resnet18/resnet50_places365.pth.tar'
            teacher_model = teacher_model.cuda() if params.cuda else teacher_model
            utils.load_checkpoint(teacher_checkpoint, teacher_model)
    else:
        if ((args.model_teacher == 'resnet18') or (args.model_teacher != 'resnet50')):
            teacher_model = wideresnet.resnet18(num_classes = int(args.num_classes))
            teacher_checkpoint = 'experiments/base_resnet18/wideresnet18_places365.pth.tar'
            teacher_model = teacher_model.cuda() if params.cuda else teacher_model
            utils.load_checkpoint_C(teacher_checkpoint, teacher_model, model_teacher=args.model_teacher, num_classes=int(args.num_classes))
        elif args.model_teacher == 'resnet50':
            teacher_model = wideresnet.resnet50(num_classes = int(args.num_classes))
            teacher_checkpoint = 'experiments/base_resnet18/resnet50_places365.pth.tar'
            teacher_model = teacher_model.cuda() if params.cuda else teacher_model
            utils.load_checkpoint_C(teacher_checkpoint, teacher_model, model_teacher=args.model_teacher, num_classes=int(args.num_classes))
